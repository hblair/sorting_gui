...
#The interpret function reads the user input for Hayden's part and feeds those values into the acquire function
    '''
    I'm gonna play around with this section. Here is the original code:
    ...
    '''
    def interpret(self):
        inputs = [self.Qhighinput.get(),self.Alowinput.get(),self.Ahighinput.get(),self.Qlowinput.get()]
        content = ['self.contentQhigh','self.contentAlow','self.contentAhigh','self.contentQlow']
        d = {}

        for i in range(0,len(inputs)):
            if inputs[i] == True:
                d[content[i]] = inputs[i]
            else:
                d[content[i]] = False

        message= '{}'.format(sf.acquire(Theory=False, Q_low=d['self.contentQlow'], Q_high=d['self.contentQhigh'], A_low=d['self.contentAlow'], A_high=d['self.contentAhigh'], Sym=False))
