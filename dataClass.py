...
##test=data('66ZN','EoL',1000000)

###########################################################Extra Functions ################################################
##This function is used in the main __init__ function to parse through strings in each line of the file inorder to find the value of
##the spin that corrospon4d to energy of that line of that file.
...
class data:##This is the main data class
    ##Enter the ENSDF file
    def __init__(self,ENSDF,option = 'EoL',energyLimit=5000,maxSpin = 9): ##Initiator to take only lines that are determined by the filt (short for filter)
        ...
        ##Extract Intitial data depending on the option.
        ##If there is a file then this program will try to extract the data.
        ...
        for line in self.f:
            ...
            if(len(line) >= 4): ##This makes sure not to take any lines that are emptry which will cause an error down the line.
            ...
            ##I have found that experimental data are one lines when c equal "L"
            ##This option ,Eo, takes all the energry values only.
            if(option == 'Eo' and a.lower() == b.lower() and c == "L"): 
                a = ''
                b = ''
                try:
                    filtDataSet.append(float(line[2]))
                except:
                    print(line[2])

            ##This option ,EoL, takes all the energry values and spin values.
